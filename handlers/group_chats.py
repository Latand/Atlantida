...
async def register_chat(message: types.Message, regexp_command):
    ...
    if 1 < timeout < 180:
        ...
        await message.answer("🏛 Просим у Вас прощения, но в данный момент длительность сеанса связи ограничена диапазоном от 2х до 180 минут (3 Часа).")
    ...
    for admin in admins:
        if chat_id == admin.user.id:
            ...
            if exists:
                ...
                text = "🏛 Поздравляем! Ваш чат связан с КАТЕГОРИЯ. Создайте новое расположение Вашего чата или выберите из предложенного списка:"
            text += "\n\n" \
...
async def asked_question(message: types.Message):
    ...
    if p and p.running:
        ...
        await message.answer("🏛 Для начала сеанса связи с КАТЕГОРИЯ введите команду /Atlantide и укажите время сеанса в минутах")
