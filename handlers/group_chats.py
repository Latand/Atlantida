...
async def register_chat(message: types.Message, regexp_command):
    ...
    if 1 < timeout < 12:
        ...
        await message.answer("🏛 Приносим извинения за временные неудобства. В данный момент длительность 🌩СеансаСвязи ограничена от 2х до 12 минут.")
...
async def asked_question(message: types.Message):
    ...
    if p and p.running:
        if p.current == "Questions":
            ...
            a = "#В(Вопросы) или #О(Ответы)" if p.current == p.QUESTIONS else "ответы к этой 🌀Мысли"
            text = f"🏛 КАТЕГОРИЯ принимает Ваши {a}, следующий 🌩СеансСвязи через {p.time_left // 60} мин\n"
            q_id = get_winner_question_id(chat_id)
            logging.info(f"QUID {q_id}")
            reply = None
            if q_id:
                text += f"🏛 Ваша КАТЕГОРИЯ ОТВЕЧАЕТ на следующую 🌀Мысль"
                reply = q_id
